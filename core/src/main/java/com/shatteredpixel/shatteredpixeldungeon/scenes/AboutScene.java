...
public class AboutScene extends PixelScene {
	...
	public void create() {
		...
		//*** Catala */
		final int GENCAT_COLOR = 0xFFFFFFFF;
		CreditsBlock gencat = new CreditsBlock(true, GENCAT_COLOR,
				null,
				Icons.GENCAT.get(),
				...
				null);
		if (landscape()){
			gencat.setRect(shpx.left(), arcnor.bottom() + 8, colWidth, 0);
		} else {
			gencat.setRect(shpx.left(), arcnor.bottom() + 8, colWidth, 0);
		}
		content.add(gencat);
		addLine(gencat.top() - 4, content);
/*
		CreditsBlock wata = new CreditsBlock(true, WATA_COLOR,
				"Pixel Dungeon",
				Icons.WATA.get(),
...
*/
		...
		transifex.setRect((Camera.main.width - colWidth)/2f, gencat.bottom() + 12, colWidth, 0);
